...
export default function(
  ...
  const _dragCallback = event => {
    ...
    if (preventHandleOutsideImage) {
      if (
        eventData.currentPoints.image.x < 0 ||
        eventData.currentPoints.image.x > eventData.image.width
      ) {
        outsideImage = true;
      }
      if (
        eventData.currentPoints.image.y < 0 ||
        eventData.currentPoints.image.y > eventData.image.height
      ) {
        outsideImage = true;
      }
