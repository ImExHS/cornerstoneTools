...
const updateCachedStats = (image, element, data) => {
  ...
  const sideA = {
    x: (data.handles.angleStart.x - data.handles.angleEnd.x) * colPixelSpacing,
  ...
  const sideA2 = {
    x:
      (data.handles.angleStart2.x - data.handles.angleEnd2.x) * colPixelSpacing,
    y:
      (data.handles.angleStart2.y - data.handles.angleEnd2.y) * rowPixelSpacing,
  };
...
const getTextBoxText = (data, rowPixelSpacing, colPixelSpacing) => {
  ...
  const d1 = external.cornerstoneMath.point
    .distance(intersectionP1, intersectionA1)
    .toFixed(2);

  const d2 = external.cornerstoneMath.point
    .distance(intersectionA2, intersectionP2)
    .toFixed(2);

  const h1 = external.cornerstoneMath.point
    .distance(intersectionP1, data.handles.perpendicularStart)
    .toFixed(2);

  const h2 = external.cornerstoneMath.point
    .distance(intersectionP2, data.handles.perpendicularStart2)
    .toFixed(2);
